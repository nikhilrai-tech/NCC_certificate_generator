...
class Certificate(models.Model):
  ...
  def apply_signature(self, role, signature_file):
        ...
        if self.final_certificate and signature_file:
            try:
                # Check if the final_certificate file exists
                if not os.path.exists(self.final_certificate.path):
                    logger.error(f"Final certificate file does not exist: {self.final_certificate.path}")
                    return

                # Check if the signature file exists
                if not os.path.exists(signature_file):
                    logger.error(f"Signature file does not exist: {signature_file}")
                    return

                # Read the existing PDF
                template_pdf = PdfReader(open(self.final_certificate.path, "rb"))
                output_pdf = PdfWriter()

                # Determine the position and page based on CertificateType and role
                if self.CertificateType.startswith('A_'):
                    position = (400, 200)
                    page = 0
                elif self.CertificateType.startswith('B_'):
                    if role == 'Staff':
                        position = (400, 220)
                        page = 0
                    elif role == 'CEO':
                        position = (100, 585)
                        page = 1
                elif self.CertificateType == 'C_Army':
                    if role == 'register_head':
                        position = (375, 100)
                        page = 0
                    elif role == 'Staff':
                        position = (300, 440)
                ...
                with Image.open(signature_file) as signature:
                    ...
                    new_data = []
                    for item in datas:
                        if item[0] > 200 and item[1] > 200 and item[2] > 200:  # Finding white color
                            new_data.append((255, 255, 255, 0))  # Making white transparent
                        else:
                            new_data.append(item)
                    signature.putdata(new_data)
                    
                    signature_reader = ImageReader(signature)
                    # Resize the signature
                    signature_width = 100  # Set the desired width
                    signature_height = int(signature.height * (signature_width / signature.width))  # Maintain aspect ratio
                    c.drawImage(signature_reader, position[0], position[1], width=signature_width, height=signature_height)

                c.save()
                buffer.seek(0)

                # Merge the new PDF with the signature into the existing PDF
                ...
                final_buffer.seek(0)
                self.final_certificate.save(f"{self.Name}_signed.pdf", ContentFile(final_buffer.getvalue()))
                logger.debug(f"Saved signed certificate PDF as {self.Name}_signed.pdf")

                # Update signature applied status
                if role == 'CEO':
                    self.ceo_signature_applied = True
                elif role == 'Staff':
                    self.staff_signature_applied = True
                elif role == 'register_head':
                    self.register_head_signature_applied = True

                # Check if all required signatures are applied
                if self.CertificateType.startswith('A_'):
                    self.is_signed = self.ceo_signature_applied
                elif self.CertificateType.startswith('B_'):
                    self.is_signed = self.ceo_signature_applied and self.staff_signature_applied
                elif self.CertificateType == 'C_Army':
                    self.is_signed = self.ceo_signature_applied and self.staff_signature_applied and self.register_head_signature_applied
